...
class HybridAgentGateway(AgentGateway):
    """Mesh HTTP pour les roles legacy ; execution in-process pour le catalogue dev."""
    ...
    def __init__(
        ...
        *,
        ...
        self._http = HttpAgentGateway(role_urls, timeout_seconds=http_timeout_seconds)
        self._local = LocalAgentGateway(model_client=model_client)
    ...
    def run_agent(
        ...
        if role in self._http.role_urls:
            return self._http.run_agent(role, work_item, memory)
        return self._local.run_agent(role, work_item, memory)
